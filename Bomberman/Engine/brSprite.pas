...
uses
  Windows, SysUtils, Classes, brGraphics,
...
type
  ...
  private
    ...
    function GetExtra(X, Y: Integer): Integer;
    procedure SetExtra(X, Y: Integer; Value: Integer);
  ...
  public
    ...
    property ChipsExtra[X, Y: Integer]: Integer read GetExtra write SetExtra;
  ...
  public
    ...
    property ImagePosX: Single read FImagePosX write FImagePosX;
    property ImagePosY: Single read FImagePosY write FImagePosY;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        c := Chips[cx2, cy2];
        if c >= 0 then
          Canvas.DrawAlpha(cx * ChipWidth + OfsX, cy * ChipHeight + OfsY, Image, c, 255);
      end;
    end;
  end else
  begin
...
function TBackgroundSprite.GetExtra(X, Y: Integer): Integer;
begin
  if (X >= 0) and (X < FMapWidth) and (Y >= 0) and (Y < FMapHeight) then
    Result := PInteger(Integer(FMapExtra) + (Y * FMapWidth + X) * SizeOf(Integer))^
  else
    Result := -1;
end;
...
procedure TBackgroundSprite.SetExtra(X, Y: Integer; Value: Integer);
begin
  if (X >= 0) and (X < FMapWidth) and (Y >= 0) and (Y < FMapHeight) then
    PInteger(Integer(FMapExtra) + (Y * FMapWidth + X) * SizeOf(Integer))^ := Value;
end;
...
begin
  ...
  begin
    ...
    ReAllocMem(FMapExtra, FMapWidth * FMapHeight * SizeOf(Integer));
    FillChar(FMapExtra^, FMapWidth * FMapHeight * SizeOf(Integer), 0);
  end;
end;
...
begin
  ...
  Pos := D3DXVector2(r.Left + ImagePosX, r.Top + ImagePosY);
